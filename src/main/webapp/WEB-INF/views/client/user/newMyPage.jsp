...
<%@ include file="/WEB-INF/views/client/common/common.jspf" %>
	...
	<body>
		<div class="container">
			...
			<c:if test="${empty newSession}">
				<form class="form-signin" id="loginForm">
				<h1>로그인을 해주세용</h1>
				</form>
			</c:if>
			<c:if test = "${not empty newSession}">
				<form id="newMyPage"> 
					<div>
						<label>이름</label>
						<label>${newSession.userName}</label>				
					</div>
					<div>
						<label>아이디</label>
						<label>${newSession.userId}</label>
					</div>
					<div>
						<label>이메일</label>
						<label>${newSession.userEmail}</label>
					</div>
					<div>
						<label>전화번호</label>
						<label>${newSession.userPhone}</label>
					</div>
					<div>
						<label>생년월일</label>
						<label>${newSession.userBirth }</label>
					</div>
					<div>
						<label>가입일</label>
						<label>${newSession.userDate }</label>
					</div>
